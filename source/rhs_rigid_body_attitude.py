...
def rhs_rigid_body_motion(t, state_vector, inertia_matrix, external_moment=np.zeros(3),
    ...
    """
        Calculation of rhs for satellite attitude equation
            Args:
                ...
                inertia_matrix (np.array[float]): diagonal matrix 3x3
    ...
    x_dot[4:7] = np.linalg.inv(inertia_matrix).dot(
        external_moment + control_torque - cross_product(omega, inertia_matrix.dot(omega) + h))
